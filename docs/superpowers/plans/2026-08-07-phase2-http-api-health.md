...
- [ ] **Step 1: Write failing test** in `backend/tests/test_health.py` that imports `handler.handler` and asserts 200 + `status=ok`
- [ ] **Step 2: Run** `cd backend && pip install -q pytest && PYTHONPATH=health pytest tests/test_health.py -v` → expect FAIL
- [ ] **Step 3: Implement** minimal `handler.py`
- [ ] **Step 4: Re-run pytest** → PASS
- [ ] **Step 5: Commit** `feat(backend): add health Lambda handler and pytest (W-200)`
...
- [ ] **Step 1: Implement module files**
- [ ] **Step 2: Commit** `feat(infra): add api module with HTTP API JWT and health route`
...
- [ ] **Step 1: Wire module.api** using cognito outputs; set `health_source_dir` to absolute or relative path that works from `envs/dev` (e.g. `${path.root}/../../../backend/health`)
- [ ] **Step 2: Run** `cd infra/envs/dev && terraform init -backend=false && terraform validate` and `terraform fmt -check -recursive ../..`
- [ ] **Step 3: Update docs**
- [ ] **Step 4: Commit** `feat(infra): wire api module in dev and update W-200 docs`
- [ ] **Step 5: Push** branch `cursor/w200-http-api-health-a099`（force-with-lease if rebased）

...
完了条件（運用側・ユーザー）: Mac で本体を再 apply したあと migrate を invoke し、admin でログインできる。
...
1. 本機能のコードを `main` に載せたあと `./infra/scripts/tf-dev.sh apply`（migrate Lambda 追加。NAT / RDS は既存）
2. migrate を invoke し `001`〜`003` を適用（`CREATE IF NOT EXISTS` で再実行可）
3. Cognito `AdminCreateUser` + グループ `admin`（username は email）
4. 同じメール / `cognito_sub` / `role=admin` / `status=active` を `users` に入れる（invoke の seed ペイロード、または手順書の SQL）
5. フロントは `frontend/.env.example` を `.env.local` にして `npm run dev`。Amplify を使う場合は `amplify_default_branch_url` を `cors_amplify_origin` に入れて再 apply

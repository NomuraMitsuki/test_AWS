...
| W-109 | GitHub Secrets（OIDC ロール ARN）登録と CI plan 有効化 | コード完了 | リポジトリ側は PR #20（bootstrap / S3 backend / `infra.yml` / 手順書）。運用側は未実施: Mac で bootstrap → `backend.hcl` 実名コミット → 本体 apply → Secrets → Environment `dev` reviewers 必須。[設計](superpowers/specs/2026-08-17-phase10-w109-remote-state-design.md) |

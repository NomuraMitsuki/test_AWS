...
| api | HTTP API, Cognito JWT authorizer, health Lambda（VPC 外・`GET /health` 認証なし）, attendance Lambda（VPC 内・JWT 必須の打刻/履歴/サマリ） |
...
3. `api`（health は `backend/health`、attendance は `backend/attendance` を zip。休暇等ドメイン Lambda は後続）

...
| S07 | 配下勤怠 | `/attendance/team` | manager, admin | 配下／全体の履歴（`records?scope=team\|all`）。行から S06 へ遷移してサマリ表示 |
...
flowchart TD
  ...
  S03 --> S04[Clock]
  S03 --> S05[History]
  S03 -->|manager_admin| S07[TeamAttendance]
  S03 --> S08[LeaveList]
  ...
  S07 --> S06
  S08 --> S09[LeaveNew]
  S08 -->|manager_admin| S10[Approvals]
```

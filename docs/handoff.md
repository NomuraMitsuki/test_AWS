...
最終更新: 2026-08-07（W-240 エクスポート API 実装後。次は W-250）

## 一言で

AWS 学習用の勤怠管理アプリ。Phase 1 Terraform、W-200（HTTP API + health）、W-210（勤怠 API）、W-220（休暇 API）、W-230（ユーザー管理 API）、**W-240（エクスポート API）** までコード実装済み（apply なし）。**次は W-250（フロント）**。W-109（GitHub Secrets / OIDC）は API・フロント完了まで着手しない。

## リポジトリ

- GitHub: `NomuraMitsuki/test_AWS`
- 作業ブランチ: **`main`**（W-240 は `cursor/w240-exports-api-a099` で実装）
- 直近マージ: PR #1〜#12
...
- WBS: W-001〜020, W-100〜108, W-006, W-200、W-210、W-220、W-230、**W-240 実装完了（apply なし・WBS ステータスは親が更新）**
- Terraform Phase 1〜6（attendance / leave / users / exports Lambda / JWT ルート）コード
...
1. **W-250**: フロント + Amplify。実装は `implementation-worker` へ委譲
2. **W-109**: **API（W-210〜）およびフロント（W-250）完了後**に再 apply → GitHub Secrets / OIDC CI（それまで着手しない）
...
次は W-250。W-109 は API・フロント完了まで着手しない。
```
